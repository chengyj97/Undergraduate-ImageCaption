...
class GenerationModel(nn.Module):
    def __init__(self, opt, word_glove):
        ...
        add skt_loss
        '''
        self.skeleton_indicate = nn.Sequential(nn.Linear(self.rnn_size, 2),  # binomial distribution
                                               nn.LogSoftmax(dim=2))
...
#        self.ctx2att = nn.Linear(self.rnn_size, self.att_hid_size)  ### att_hid_size = 512
    ...
    def forward(self, img, gt_seqs, input_seqs, gt_lemma, gt_stops, cat_embeds, \
                img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_, num, gt_seqs_lens, option='MLE', eos_idx = 0):
        if option == 'MLE':
            return self._forward(img, gt_seqs, input_seqs, gt_lemma, gt_stops, cat_embeds, \
                                 img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_, num, gt_seqs_lens)
        elif option == 'sample':  # for validation
            if self.beam_size == 1:
                return self._sample(img, gt_seqs, input_seqs, gt_lemma, gt_stops, cat_embeds, \
                                 img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_, num, gt_seqs_lens)
            else:
                return self._sample_beam(img, gt_seqs, input_seqs, cat_embeds, \
                                 img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_, num, gt_seqs_lens, eos_idx)
    ...
    add skt_loss
    '''
    def lcs(self, gt_stops_copy, gt_seq_, gt_lemma_, gt_stops_):
        gt_skeleton = gt_seq_.data.new(gt_seq_.shape).zero_()
        for batch in range(gt_stops_copy.shape[0]):
            seq = gt_seq_[batch]; lem = gt_lemma_[batch]
            skt = torch.zeros(seq.shape).long().to(self.device)
            x = gt_stops_copy[batch]; y = gt_stops_[batch]
            x = torch.take(x,torch.nonzero(x)).squeeze() if len(torch.nonzero(x))>1 else torch.take(x,torch.nonzero(x)).squeeze().unsqueeze(0)
            y = torch.take(y,torch.nonzero(y)).squeeze()
            x_ = torch.zeros(x.shape).long().to(self.device).data.copy_(x)
            y_ = torch.zeros(y.shape).long().to(self.device)
            for i,item in enumerate(y):
                temp = (x_==item).nonzero()
                if len(temp) > 0:
                    y_[i] = item  # item
                    x_ = x_[temp[0]+1:]
            y_ = torch.take(y_,torch.nonzero(y_))
            for i,item in enumerate(lem):
                if item in y_:
                    skt[i] = seq[i]
            gt_skeleton[batch] = skt
        return gt_skeleton
    ...
    def _forward(self, img, gt_seqs, input_seqs, gt_lemma, gt_stops, cat_embeds, \
                img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_, num, gt_seqs_lens):        
        ...
        skeleton_probs = self.skeleton_indicate(skeleton_hidden)
        show_skeleton = skeleton_probs[:,:,1] >= 0.5
        
        skeleton_probs = skeleton_probs.unsqueeze(1).expand(batch_size, self.seq_per_img, skeleton_probs.shape[1], skeleton_probs.shape[2])\
                            .contiguous().view(-1, skeleton_probs.shape[1], skeleton_probs.shape[2])
        gt_seq_ = gt_seq_.unsqueeze(1).expand(batch_size, self.seq_per_img, gt_seq_.shape[1])\
                    .contiguous().view(-1, gt_seq_.shape[1])  # (batch * 5, 16)
        gt_lemma_ = gt_lemma_.unsqueeze(1).expand(batch_size, self.seq_per_img, gt_lemma_.shape[1])\
                    .contiguous().view(-1, gt_lemma_.shape[1])  # (batch * 5, 16)
        gt_stops_ = gt_stops_.unsqueeze(1).expand(batch_size, self.seq_per_img, gt_stops_.shape[1])\
                    .contiguous().view(-1, gt_stops_.shape[1])  # (batch * 5, 16)
        gt_stops = gt_stops.view(-1, gt_seqs.shape[1])  # (batch * 5, seq_length)
        
        
        gt_seq_lens_ = torch.LongTensor([len(seq.nonzero()) for seq in gt_seq_]).to(self.device)
        gt_seq_lens_, sort_ind = gt_seq_lens_.sort(dim=0, descending=True)
        skeleton_probs = skeleton_probs[sort_ind]
        gt_seq_ = gt_seq_[sort_ind]
        gt_lemma_ = gt_lemma_[sort_ind]
        gt_stops_ = gt_stops_[sort_ind]        
        gt_stops_copy = gt_stops[sort_ind]  ###
        
        gt_skeleton = self.lcs(gt_stops_copy, gt_seq_, gt_lemma_, gt_stops_) > 0  # convert from gt_seq_
        
        #gt_seq_lens_ = gt_seq_lens_.tolist()
        skeleton_probs, _ = pack_padded_sequence(skeleton_probs, gt_seq_lens_, batch_first=True)
        gt_skeleton, _ = pack_padded_sequence(gt_skeleton, gt_seq_lens_, batch_first=True)
        
        skt_loss = self.criterion(skeleton_probs, gt_skeleton.long())  ### !!!
        #_, desorted_indices = torch.sort(sort_ind, descending=False)
        #skt_loss = skt_loss[desorted_indices]
...
#        p_conv_feats = self.ctx2att(conv_feats)  # (batch * 5, 16 * 16, att_hid_size = 512)
        ...
        add skt_loss
        '''
        #skt_loss = skt_loss[sort_ind]
        return loss, show_skeleton, skt_loss


    #------------------------------- validate ---------------------------------    
    
    def _sample(self, img, gt_seqs, input_seqs, gt_lemma, gt_stops, cat_embeds, \
                img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_, num, gt_seqs_lens):
        '''
        ...
        return input_seqs, show_skeleton, gt_skeleton
    ...
    def _sample_beam(self, img, gt_seqs, input_seqs, gt_lemma, gt_stops, cat_embeds, \
                img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_, num, gt_seqs_lens, eos_idx):

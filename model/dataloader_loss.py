...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 18 13:03:22 2019
...
class DataLoader(Dataset):
    def __init__(self, opt, imgids, coco_cap, coco_cap_lemma, catnm_glove, cocotools_det, cocotools_det_train, coco_det_train, device, split='train'):
        '''
        ...
        self.coco_cap_lemma = coco_cap_lemma  ###
    ...
    def __getitem__(self, index):
        ...
        '''for skt_loss'''
        coco_cap_ls = self.coco_cap_lemma[imgid]
        gt_lemma = torch.LongTensor(coco_cap_ls['lemma'])[:,1:].to(self.device)  ### !!! lemma_idx & word_idx are different
        gt_stops = torch.LongTensor(coco_cap_ls['stops'])[:,1:].to(self.device)
        ...
        idx = random.randint(0, gt_seqs_.shape[0]-1)
        gt_seq_ = gt_seqs_[idx,1:]  # with only eos (16)

        '''for skt_loss'''
        coco_cap_ls = self.coco_cap_lemma[imgid_]
        gt_lemma_ = torch.LongTensor(coco_cap_ls['lemma'])[idx,1:].to(self.device)
        gt_stops_ = torch.LongTensor(coco_cap_ls['stops'])[idx,1:].to(self.device)
        ...
        return imgid, imgid_, num, \
                ...
                img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_

...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 18 13:02:01 2019
...
from model.model_loss import GenerationModel  ###
from model.dataloader_loss import DataLoader  ###
...
#    for step in range(2000):
        imgid, imgid_, num, \
                img, gt_seqs, input_seqs, gt_lemma, gt_stops, cat_embeds, \
                img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_ = data_iter_train.next()
        gt_seqs_lens = torch.LongTensor([[len(seq.nonzero()) for seq in batch] for batch in gt_seqs]).to(device).unsqueeze(2)  # (batch, 5, 1)
        
            
        loss = 0        
        lm_loss, show_skeleton, skt_loss = model(img, gt_seqs, input_seqs, gt_lemma, gt_stops, cat_embeds, \
                                       img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_, num, gt_seqs_lens, 'MLE')        
        loss += (lm_loss+skt_loss)
        ...
        skt_loss_temp += skt_loss
                
        if (step+1) % opt.disp_interval == 0 and step != 0:
        #if step>=0:
            end = time.time()
            lm_loss_temp /= opt.disp_interval
            print("step {}/{} (epoch {}), lm_loss = {:.3f}, lm_loss(avg) = {:.3f}, skt_loss = {:.3f}, skt_loss(avg) = {:.3f}, lr = {:.5f}, time = {:.3f}" \
                .format(step+1, len(dataloader_train), epoch, lm_loss, lm_loss_temp, skt_loss, skt_loss_temp, opt.learning_rate, end-start))
...
#    for step in range(500):
        imgid, imgid_, num, \
                img, gt_seqs, input_seqs, gt_lemma, gt_stops, cat_embeds, \
                img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_ = data_iter_val.next()
        gt_seqs_lens = torch.LongTensor([[len(seq.nonzero()) for seq in batch] for batch in gt_seqs]).to(device).unsqueeze(2)  # (batch, 5, 1)
        ...
        input_seqs, show_skeleton, gt_skeleton = model(img, gt_seqs, input_seqs, gt_lemma, gt_stops, cat_embeds, \
                                          img_, gt_seq_, gt_lemma_, gt_stops_, cat_embeds_, num, gt_seqs_lens, 'sample', word_idx['<eos>'])  ### !!!

        show_skeleton = (gt_seq_.float() * show_skeleton.float()).long().unsqueeze(1)\
                            .expand(opt.batch_size, model.seq_per_img, show_skeleton.shape[1])\
                            .contiguous().view(-1,show_skeleton.shape[1])  # (batch * 5, 16)
        gt_skeleton = (gt_seq_.unsqueeze(1).expand(opt.batch_size, model.seq_per_img, show_skeleton.shape[1])\
                            .contiguous().view(-1,show_skeleton.shape[1]).float() * gt_skeleton.float()).long()
        skeletons['gt'].append(gt_skeleton.tolist())
        skeletons['generated'].append(show_skeleton.tolist())
...
#    coco_cap_test = coco_cap['coco_cap_test']
    ...
    for skt_loss
    '''
    opt.coco_caption_json_lemma_path = './data/coco_caption2_lemma.json'
    coco_cap_lemma = json.load(open(opt.coco_caption_json_lemma_path, 'r'))
...
#                    [i for i in list(coco_cap_test.keys()) if cocotools_det_val.getAnnIds(imgIds=int(i)) != []]
    ...
    ### data loader
    dataset_train = DataLoader(opt, imgids_train, coco_cap, coco_cap_lemma, catnm_glove, cocotools_det_train, \
    ...
    dataset_val = DataLoader(opt, imgids_val, coco_cap, coco_cap_lemma, catnm_glove, cocotools_det_val, \
...
#    dataset_test = DataLoader(opt, imgids_test, coco_cap, coco_cap_lemma, catnm_glove, cocotools_det_val, \
